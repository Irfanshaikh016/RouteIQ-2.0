...
The keys live in `.claude/settings.json` (project) or `C:/Users/irfan/Downloads/RouteIQ-2.0/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @C:/Users/irfan/Downloads/RouteIQ-2.0/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
